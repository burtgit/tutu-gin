...
func (c *Client) videoFromIDV2(ctx context.Context, id string) (*Video, error) {
	c.assureClient()
	html, err := c.httpGetBodyBytes(ctx, "https://www.youtube.com/watch?v="+id+"&bpctr=9999999999&has_verified=1")
	if err != nil {
		return nil, err
...
func (c *Client) httpPost(ctx context.Context, url string, body interface{}) (*http.Response, error) {
	...
	if resp.StatusCode != http.StatusOK {
		resp.Body.Close()
		return nil, errors2.Annotate(ErrUnexpectedStatusCode(resp.StatusCode), ErrUnexpectedStatusCode(resp.StatusCode).Error())
	}
